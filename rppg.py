...
def pos_algorithm(rgb_raw, fps, window_s=1.6):
    ...
    S = np.zeros(n)
    overlap_count = np.zeros(n)  # vetor para corrigir o início/fim das janelas
    ...
    for start in range(0, n - window_len + 1):
        ...
        # remove média da janela (sem dividir pelo std - tava errado)
        h = h - np.mean(h)
        S[start:end] += h
        overlap_count[start:end] += 1.0

    # normaliza as regiões de borda pelo número real de sobreposições
    overlap_count[overlap_count == 0] = 1.0
    S = S / overlap_count

    # remove tendência linear do sinal final
    S = sig.detrend(S, type='linear')
...
def combine_roi_and_methods(roi_signals, fps):
    ...
    for roi_name, rgb_raw in roi_signals.items():
        ...
        # CHROM requer normalização local ou global adequada
        rgb_norm = preprocess_rgb_signal(rgb_smooth)
        
        s_chrom = chrom_algorithm(rgb_norm)
        s_pos = pos_algorithm(rgb_smooth, fps)

        # normalização para depois combinar os dois
